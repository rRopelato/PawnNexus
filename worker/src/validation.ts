...
const platforms = new Set(['Steam', 'Nintendo Switch', 'Nintendo Switch 2', 'PlayStation', 'Xbox']);
...
export function validatePawnImages(value: unknown) {
  ...
  return value.map((item, index) => {
    ...
    if (!isAllowedPawnImageUrl(imageUrl) || !isAllowedPawnImageUrl(thumbUrl)) {
      throw new HTTPException(400, { message: 'pawn image URLs must be generated by PawnNexus uploads' });
    }
...
export function validatePawnPayload(body: Record<string, unknown>) {
  const platform = normalizePlatform(requireString(body.platform, 'platform'));
  ...
  const specialization = optionalString(body.specialization, 'specialization', 120);
  if (specialization && !specializations.has(specialization)) throw new HTTPException(400, { message: 'specialization is invalid' });

  const images = validatePawnImages(body.images);
  const weaponSkills = normalizeWeaponSkills(body.weaponSkills ?? body.skills);
  ...
  return {
    ...
    psnId,
    xboxGamertag,
    weapon1: optionalString(body.weapon1, 'weapon1', 120),
    weapon2: vocation === 'Fighter' ? optionalString(body.weapon2, 'weapon2', 120) : null,
    head: optionalString(body.head, 'head', 120),
    body: optionalString(body.body, 'body', 120),
    legs: optionalString(body.legs, 'legs', 120),
    cloak: optionalString(body.cloak, 'cloak', 120),
    ring1: optionalString(body.ring1, 'ring1', 120),
    ring2: optionalString(body.ring2, 'ring2', 120),
    augment1: optionalString(body.augment1, 'augment1', 120),
    augment2: optionalString(body.augment2, 'augment2', 120),
    augment3: optionalString(body.augment3, 'augment3', 120),
    augment4: optionalString(body.augment4, 'augment4', 120),
    augment5: optionalString(body.augment5, 'augment5', 120),
    augment6: optionalString(body.augment6, 'augment6', 120),
    specialization,
    images,
...
function isAllowedPawnImageUrl(url: string) {
  return allowedPawnImageUrlPrefixes.some((prefix) => url.startsWith(prefix));
}

function normalizeWeaponSkills(value: unknown) {
  const source = Array.isArray(value) ? value : typeof value === 'string' ? value.split('\n') : [];
  const weaponSkills = source
